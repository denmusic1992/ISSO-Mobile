...
public class SettingsActivity extends PreferenceActivity  implements CustomExpandListViewAdapter.customDownloadListener,
        CustomExpandListViewAdapter.customDeleteListener {
    ...
    public class SettingsFragment extends PreferenceFragment implements SharedPreferences.OnSharedPreferenceChangeListener {
        ...
        public View onCreateView(LayoutInflater inflater, ViewGroup container,
            ...
            listPreference = (CustomListPreference) findPreference("listPref");

            ConnectivityManager connManager = (ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
            final NetworkInfo mWifi = connManager.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
            CheckBoxPreference mCheckBoxPrefPhoto = (CheckBoxPreference) findPreference("AllowToDownloadPhotos");
            CheckBoxPreference mCheckBoxPrefScheme = (CheckBoxPreference) findPreference("AllowToDownloadSchemes");
            if (mWifi.isConnected()) {
                mCheckBoxPrefPhoto.setChecked(true);
                mCheckBoxPrefScheme.setChecked(true);
            }
            else {
                mCheckBoxPrefPhoto.setChecked(false);
                mCheckBoxPrefScheme.setChecked(false);
            }


            v.addView(btn);
            CharSequence[] entries = new CharSequence[collections.length];
            CharSequence[] entryValues = new CharSequence[collections.length];
            for(int i = 0; i < collections.length; i++) {
                entries[i] = collections[i].CollectionName + " (" + collections[i].IssoList.split(",").length + ") от " +
                        getDate(collections[i].DateCreate);
            ...
            btn.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {
                    /*final CheckBoxPreference checkPhoto = (CheckBoxPreference) findPreference("AllowToDownloadPhotos");
                    final CheckBoxPreference checkScheme = (CheckBoxPreference) findPreference("AllowToDownloadSchemes");
                    if(!mWifi.isConnected()) {
                        ...
                        builder3.setMessage("Вы не подключены к сети wi-fi. Загрузка будет осуществляться через мобильный интернет. Продолжить?")
                                ...
                                .setPositiveButton("Да", new DialogInterface.OnClickListener() {
                                    public void onClick(DialogInterface dialog, int id) {
                                        startSync(checkPhoto != null ? checkPhoto.isChecked() : true, checkScheme != null ? checkScheme.isChecked() : true);
                                    }
                                })
                    ...
                    onDownload(collections[PreferenceManager.getDefaultSharedPreferences(SettingsActivity.this).getInt("index", -1)]);
                }
            });
            if (sm != null) {
                CheckBoxPreference checkPhoto = (CheckBoxPreference) findPreference("AllowToDownloadPhotos");
                CheckBoxPreference checkScheme = (CheckBoxPreference) findPreference("AllowToDownloadSchemes");
                startSync(checkPhoto != null ? checkPhoto.isChecked() : true, checkScheme != null ? checkScheme.isChecked() : true, collection);
    ...
    @Override
    public void onDelete(UserCollection collection) {

    }

    public void startSync(boolean downloadPhoto, boolean downloadScheme, UserCollection collection) {
        String login = PreferenceManager.getDefaultSharedPreferences(SettingsActivity.this).getString("Login", null);
        String pass = PreferenceManager.getDefaultSharedPreferences(SettingsActivity.this).getString("Pass", null);
        ...
        if(sm == null) {
            ...
            sm.dbHelper = new DBHelper(SettingsActivity.this);
    ...
    public Handler HandlerForConnect = new Handler() {
        public void handleMessage(Message msg) {
            collections= (UserCollection[]) msg.obj;
            if(collections != null && collections.length > 0) {
                ...
                listPreference.setEnabled(true);
                /*AlertDialog.Builder bld = new AlertDialog.Builder(SettingsActivity.this);
                View view = getLayoutInflater().inflate(R.layout.collection_layout, null);
                TableLayout table = (TableLayout) view.findViewById(R.id.tableDownloadList);
                if (MainActivity.theme.equals("0"))
                    table.setBackground(getDrawable(R.drawable.cell_borders_dark));
                else
                    table.setBackground(getDrawable(R.drawable.cell_borders_light));
                TableRow.LayoutParams l = new TableRow.LayoutParams(TableRow.LayoutParams.MATCH_PARENT, TableRow.LayoutParams.MATCH_PARENT);
                FillDownloadTable(issos, table);

                *//** Expandable listview *//*
            *//*ExpandableListView lv = (ExpandableListView) view.findViewById(R.id.lvExpandableCollection);
            CustomExpandListViewAdapter adapter = new CustomExpandListViewAdapter(getApplicationContext(), issos);
            adapter.setCustomDownloadListener(SettingsActivity.this);
            adapter.setCustomDeleteListener(SettingsActivity.this);
            lv.setAdapter(adapter);*//*

                *//** Custom dialog *//*
            *//*Dialog dialog = new Dialog(getApplicationContext());
            CustomListAdapter adapter = new CustomListAdapter(getApplicationContext(), issos);
            SlideExpandableListAdapter listAdapter = new SlideExpandableListAdapter(adapter, R.id.expandable_toggle_button, R.id.expandable);
            ...
            dialog.show();*//*
                ...
                AlertDialog.Builder bld = new AlertDialog.Builder(SettingsActivity.this);
                bld.setMessage("Не удалось получить список выгрузок для данного пользователя.");
                AlertDialog alert = bld.create();
                alert.show();
                doKeepDialog(alert);
    ...
    public void FillDownloadTable(UserCollection[] collections, TableLayout table) {
        ...
        for(final UserCollection collection : collections) {
            ...
            tr.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(SettingsActivity.this, AdvancedDownloadInfo.class);
    ...
    class SyncModule extends AsyncTask<Void, String, Void> {
        ...
        private void doJSONSync() {
            ...
            try {
                ...
                params = new HashMap<>();
                params.put("id", id);
                object = ConnectToServer(JSONMETHODS[1], params);
                dialog.setProgress(0);
                ...
                for (HttpsIsso i : issos) {
                    ...
                    dialog.setProgress(((bibs / issos.length) * 100));
                    publishProgress(String.valueOf(((bibs / issos.length) * 100)), "Шаг 2 из " + countOfSteps + ". Получение ИССО...");
                    cIsso.put(i.CIsso);
                ...
                object = ConnectToServer(JSONMETHODS[11], params);
                dialog.setProgress(0);
                ...
                for (HttpsTableNames names : meanInfo.tableNames) {
                    ...
                    if(!names.IsTableExportedWhole) {
                        ...
                        object = ConnectToServer(JSONMETHODS[12], params);
                        if(isCancelled())
                            return;
                        gson = new Gson();
                        t = System.currentTimeMillis() - t;
                ...
                params = new HashMap<>();
                params.put("id", id);
                object = ConnectToServer(JSONMETHODS[13], params);
                if(isCancelled())
                    return;
                ...
                object = ConnectToServer(JSONMETHODS[3], params);
                if(isCancelled())
                    return;
                JSONArray res = object.getJSONArray(JSONMETHODRESULTS[3]);
                db = new DBHelper(SettingsActivity.this).getWritableDatabase();
                ...
                for (String table : DEFECTS) {
                    ...
                    if(!table.substring(0, 1).equals("I") && !table.substring(0, 1).toLowerCase().equals("v")) {
                        ...
                        cr.close();
                        object = ConnectToServer(JSONMETHODS[14], params);
                        if(isCancelled())
                            return;
                        ...
                        if(isActual == 0) {
                            ...
                            object = ConnectToServer(JSONMETHODS[4], params);
                            if(isCancelled())
                                return;
                            t = System.currentTimeMillis() - t;
                            time1 += t;
                        ...
                        for(int index = 0; index < issos.length; index += 20) {
                            ...
                            object = ConnectToServer(JSONMETHODS[15], params);
                            if(isCancelled())
                                return;
                            t = System.currentTimeMillis() - t;
                            time1 += t;
                ...
                ConnectToServer(JSONMETHODS[10], params);
        ...
        public int MainConstrId(int c_gr_constr) {
        ...
        public void doPhotoSync(int countOfSteps, HttpsIsso[] issos, String issoList) {
            try {
                SQLiteDatabase db = new DBHelper(SettingsActivity.this).getReadableDatabase();
                db.beginTransaction();
                ...
                for(int index = 0; index < issos.length; index += 20) {
                    ...
                    Log.d("Tag", "doing for isso = " + array.getString(0));
                    JSONObject object = ConnectToServer(JSONMETHODS[15], params);
                    if (isCancelled())
                        return;
                    t = System.currentTimeMillis() - t;
                    time1 += t;
                    ...
                    for(n = 0; n < 20 && n + index < issos.length; n++ ) {
                        ...
                        if (isCancelled())
                            return;
                        Cursor dbCursor = new DBHelper(SettingsActivity.this).getReadableDatabase().query("UPLOAD_PHOTOS", null, null, null, null, null, null);
                        String[] columnNames = dbCursor.getColumnNames();
                        String[] questionMarks = new String[columnNames.length];
                        Arrays.fill(questionMarks, "?");
                        dbCursor.close();
        ...
        public void doSchemeSync(int countOfSteps, HttpsIsso[] issos, String issoList) {
            try {
                SQLiteDatabase db = new DBHelper(SettingsActivity.this).getReadableDatabase();
                db.beginTransaction();
                ...
                for(int index = 0; index < issos.length; index += 20) {
                    ...
                    for(n = 0; n < 20 && n + index < issos.length; n++ ) {
                        ...
                        if (isCancelled())
                            return;
                        Cursor dbCursor = new DBHelper(SettingsActivity.this).getReadableDatabase().query("UPLOAD_SCHEMES", null, null, null, null, null, null);
                        String[] columnNames = dbCursor.getColumnNames();
                        String[] questionMarks = new String[columnNames.length];
                        Arrays.fill(questionMarks, "?");
                        dbCursor.close();
        ...
        public JSONArray GetInformationDefects(String tableLoc, String tableServ, String pks, List<String> list, JSONArray array) {
            try {
                Cursor cr;
                SQLiteDatabase db = new DBHelper(SettingsActivity.this).getReadableDatabase();
                ...
                params.put("clientPks", result);
                params.put("cIssos", array);
                JSONObject object = ConnectToServer(JSONMETHODS[16], params);
                t = System.currentTimeMillis() - t;
                time1 += t;
        ...
        private boolean isTableExists(String tableName, int c_isso) {
            Cursor c = new DBHelper(SettingsActivity.this).getReadableDatabase().rawQuery("SELECT DISTINCT tbl_name from sqlite_master where tbl_name = '" + tableName + "'", null);
            if(c != null) {
                if(c.getCount() > 0 ) {
                    if(c_isso < 0) {
                        c.close();
                        ...
                        Cursor cr = new DBHelper(SettingsActivity.this).getReadableDatabase().rawQuery("select * from " + tableName + " where C_ISSO = " + c_isso, null);
                        if(cr.getCount() > 0) {
                            cr.close();
                            return true;
                        }
